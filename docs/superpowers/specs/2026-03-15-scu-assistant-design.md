...
**开发期（单机 Docker Compose）：** 所有业务服务（academic / food / campus / crawler）作为 Gateway 进程内的模块运行，共享同一个 FastAPI 实例。`ai-svc` 作为独立进程部署（因资源消耗大、启动慢）。即 Docker Compose 实际运行 3 个应用容器：`frontend`、`gateway`（含业务模块）、`ai-svc`。

**如需扩展（可选）：** 各业务模块可拆为独立进程，只需改 Docker Compose 配置和路由表，代码无需修改（每个 service 目录已有独立的 router 注册）。

Section 6.1 的项目结构和 Section 8.3 的 Docker Compose 均按此模型理解。
...
**教务系统不可用时：** 返回 `503 Service Unavailable`，提示"教务系统暂时无法访问，请稍后再试"。已登录用户不受影响（JWT 独立验证）。
...
| GET | `/api/academic/course-recommend` | 选课推荐 | `?semester=` (用户已修课程和培养方案从用户档案自动获取) | `{ required: [{ name, credits, reason }], elective: [{ name, credits, reason }], conflicts: [{ course_a, course_b, conflict_type: "time_overlap" }] }` |
...
**爬虫流程：**
```
crawler-svc 定时触发 → 抓取目标页面 → 解析结构化数据
  → 写入 PostgreSQL（通过 shared 数据库连接，与业务服务共享 DB）
  → 发布 Redis Streams 事件通知业务服务刷新缓存
```

**错误处理：** 抓取失败时重试 2 次（间隔 30s），仍失败则记录日志并跳过本轮，不影响下次调度。

**数据初始化：** 食堂、校车、校历等静态数据通过 Alembic 数据库迁移的 seed 脚本初始化（JSON fixtures → SQL INSERT）。

### 6.8 数据库迁移

使用 Alembic 管理数据库 schema 变更，所有 migration 文件纳入版本控制。团队成员拉取代码后执行 `alembic upgrade head` 即可同步数据库。
...
- **Embedding：** `text-embedding-v3`（通义千问），与主模型同生态
...
**记忆提取规则：**
- **触发时机：** 每次对话结束时（用户 10 分钟无新消息，或主动结束对话）
- **提取方式：** 将对话内容交给 LLM，提示词要求提取用户偏好/信息，输出结构化 KV 对
- **冲突解决：** 新记忆覆盖旧记忆（同 key），置信度取较高值。如"不吃辣"覆盖之前的"喜欢辣"
- **上限：** 每用户最多 50 条记忆，超出时按置信度 + 更新时间淘汰最旧最低的
...
services:
  ...
  gateway:       # FastAPI Gateway + 业务模块 (academic/food/campus/crawler), port 8000
  ai-svc:        # AI 服务 (llm-gateway + rag-engine + intent-router), port 8005
  postgres:      # port 5432
  redis:         # port 6379
  milvus:        # port 19530 (P1 阶段 RAG 功能启用时加入)
```
